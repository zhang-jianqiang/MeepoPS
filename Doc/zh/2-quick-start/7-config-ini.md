# 配置文件
- FastWS的配置文件路径FastWS/config.php
- FastWS的配置文件采用php.ini的形式.
- 修改后需要重启FastWS.

```
; FastWS configuration file
; FastWS configuration file as in php.ini
; FastWS configuration file comments start with ';'
...
log_filename_prefix = '/var/log/fast_ws/fast_ws_'

; 标准输出文件路径. Debug模式下的标准输出就是显示器.
stdout_path_prefix = '/var/log/fast_ws/fast_ws_'

; 主进程的Pid文件存放路径
master_pid_path = '/var/run/fast_ws/fast_ws_master.pid'

; FastWS对子进程的统计信息收集存放的文件(该文件是一次性的)
statistics_path = '/var/run/fast_ws/fast_ws_statistics'
...
; Select轮询机制最大的监听资源数
; 这是PHP内核的限制. 默认是1024. 
; FastWS将限制1020. 因为我们在避免空轮训时加入了无意义的Socket资源, 因此这个值必须比FD_SETSIZE少4
; 如果我们想改变这个值, 必须重新编译PHP, 例如"--enable-fd-setsize=2048"
; PHP的FD_SETSIZE最大可以等于操作系统所限制的打开文件最大数
; 请注意: 比如您重新编译了FD_SETSIZE为2048, 但是FastWS并不能感知这个值, 所以需要手动设置.
